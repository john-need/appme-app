import query from "./fetch-time-entries";
...
describe("fetch-time-entries", () => {
  ...
  it("returns [] on 404", async () => {
    ...
    await expect(query("token" as unknown as string)).resolves.toEqual([]);
  });

  it("throws on non-ok status with message", async () => {
    g.fetch.mockResolvedValue({ ok: false, status: 500, text: async () => "err" });
    await expect(query("t" as unknown as string)).rejects.toThrow(
